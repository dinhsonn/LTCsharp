...
namespace do_an.View.List
{
    public partial class frmCustomer__List : Form
    {
        ...
        private void dgvList_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                ShowMessagebox.Error("Chưa chọn khách hàng!");
                return;
            }
        ...
        private void listLoad()
        {
            ...
            {
                setList();
                lblResult.Text = "Kết quả tìm thấy: " + new bCustomer().GetCount(tbSearch.Text.Trim()).ToString();
            }
            catch (Exception ex)
            {
        ...
        {
            IEnumerable<Customer> data = new bCustomer().GetList(tbSearch.Text.Trim());
            ...
            dgvList.Rows.AddRange(data.Select((i, count) => new DataGridViewRow
            {
                Cells =
                {
                    ...
                    new DataGridViewTextBoxCell { Value = i.Phone },
        ...
        private void deleteRows()
        {
            ...
            if (ShowMessagebox.Question($"Bạn có muốn xoá khách hàng {String.Join(",", listSelectRowFromDGV.ToArray())} không?") == DialogResult.No)
                return;

            List<Customer> bak = new List<Customer>();

            foreach (var i in listSelectRowFromDGV)
            {
                bool rs = new bCustomer().Delete(i);

                if (!rs)
                {
                    bak.ForEach(e => new bCustomer().Add(e));
                    ShowMessagebox.Error($"Không thể xoá {i}: {new bCustomer().GetDetail(i).Name}!");
                    return;
                }

                bak.Add(new bCustomer().GetDetail(i));
            }
            listLoad();
        }
        ...
        private void openForm(bool isView, bool isAdd)
        {
            if (isAdd)
            {
                using (var frm = new frmCustomer__Detail(isAdd))
                {
                    frm.ShowDialog();
            ...
            using (var frm = new frmCustomer__Detail(code__selected, isView))
            {
                frm.ShowDialog();
                if (frm.DialogResult == DialogResult.Yes)
                    listLoad();
