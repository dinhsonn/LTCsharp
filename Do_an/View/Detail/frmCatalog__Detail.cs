...
namespace do_an.View.Detail
{
    public partial class frmCatalog__Detail : Form
    {
        ...
        public frmCatalog__Detail(string Code, bool IsViewOrEdit)
        {
            InitializeComponent();
        ...
        {
            catalog = new bCatalog().GetDetail(code);
            tbCode.Text = catalog.CatalogCode;
            tbName.Text = catalog.Name;
        }
        ...
        {
            package();
            bCatalog b = new bCatalog();
            string mess = $"{catalog.CatalogCode}: {catalog.Name}";

            if (b.CheckExists(catalog.CatalogCode) && isAdd)
            {
                var d = b.GetDetail(catalog.CatalogCode);
                mess = $"Đã tồn tại {catalog.CatalogCode}\n{d.CatalogCode}: {d.Name}";
                ShowMessagebox.Error(mess);
                return;
            }

            bool result = isAdd ? b.Add(catalog) : b.Update(catalog);
        ...
        private void delete()
        {
            ...
            if (!b.CheckExists(catalog.CatalogCode))
            {
                ShowMessagebox.Error($"Đã tồn tại {mess}!");
                return;
            }

            bool result = b.Delete(catalog.CatalogCode);
